...
function ht(){
        if (prix.value != '' && qota.value != ''){
               let  result =(  +prix.value *+qota.value)-+remise.value;
               montant.innerHTML =+ result;
               montant.style.background ='#040';
        }else{
montant.innerHTML ='Montant ht:';
montant.style.background ='#d10e1e';}       
};
//creer un produit
sub.onclick =function (){

        if(qota.value != '' &&  pro.value !='' && remise.value!='' ){
                let newdwa  =  {
                        pro :pro.value,qota:qota.value,remise:remise.value,prix:prix.value,montant:montant.innerHTML       
                        };
                        dwa.push(newdwa); 
                        localStorage.setItem('product',JSON.stringify(dwa));
                        pro.value=''  ;qota.value='' ;remise.value=''; prix.value='' ;montant.innerHTML='Montant ht' ;montant.style.background ='#d10e1e';
